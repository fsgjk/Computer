...
var clientIP = '';
var dataSyncReady = false;
var dataSyncQueue = [];

function onDataReady(cb) {
  if(dataSyncReady) { cb(); return; }
  dataSyncQueue.push(cb);
}
function fireDataReady() {
  dataSyncReady = true;
  for(var i=0;i<dataSyncQueue.length;i++) dataSyncQueue[i]();
  dataSyncQueue = [];
}
  ...
  function useLocalFallback() {
    try {
      ...
      if(saved) {
        ...
        console.log('从localStorage加载:', DATA.length, '条');
      }
    } catch(e) {}
    if(!DATA.length) {
...
function refreshLogPanel() {
  ...
  list.innerHTML = recent.map(function(l) {
    var icon = l.action==='新增'?'➕':l.action==='删除'?'🗑':'✏';
    var color = l.action==='新增'?'#3B6D11':l.action==='删除'?'#A32D2D':'#185FA5';
    return '<div class="log-item"><span class="log-time">['+l.time+']</span><span style="color:'+color+';font-weight:500">'+icon+' '+l.action+'</span><span style="color:var(--neutral-800)">'+l.target+'</span>'+(l.detail?'<div style="font-size:10px;color:var(--neutral-400);margin-top:2px;padding-left:4px">'+l.detail+'</div>':'')+(l.ip?'<span class="log-ip">IP:'+l.ip+'</span>':'')+'</div>';
  }).join('');
}
